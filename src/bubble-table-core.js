(function (root, factory) {
  ...
  var defaults = {
    ...
    style: {
      ...
      toolbarButtonText: "#111827",
      toolbarButtonHoverBackground: "#eff6ff",
      toolbarButtonBorder: "#d1d5db",
      toolbarButtonRadius: 4,
      toolbarButtonFontSize: 12,
      toolbarGap: 6,
      tableBackground: "#ffffff",
      tableShadow: "0 8px 24px rgba(15, 23, 42, 0.08)"
    }
  };
  ...
  function validateAlign(value, fallback) {
    if (value == null || value === "") return fallback;
  ...
  function normalizeConfig(input) {
    ...
    var styleOverrides = config.styleOverrides || {};
    var style = {};
    ...
    if (isPlainObject(styleOverrides)) {
      Object.keys(styleOverrides).forEach(function (key) {
        if (styleOverrides[key] != null && styleOverrides[key] !== "") {
          style[key] = styleOverrides[key];
        }
      });
    }
    ...
    return {
      ...
      defaultColumnAlign: validateAlign(config.defaultColumnAlign, defaults.defaultColumnAlign),
      defaultHeaderAlign: validateAlign(config.defaultHeaderAlign, defaults.defaultHeaderAlign),
      defaultColumnWidth: asNumber(config.defaultColumnWidth, defaults.defaultColumnWidth),
      headerHeight: asNumber(config.headerHeight, defaults.headerHeight),
      showGrid: asBoolean(config.showGrid, defaults.showGrid),
  ...
  function buildStyleText(id, style, layout) {
    layout = layout || {};
    var rootClass = ".bst-" + id;
    var toolbarGap = asSize(style.toolbarGap, defaults.style.toolbarGap);
    var cellPaddingX = asSize(style.cellPaddingX, defaults.style.cellPaddingX);
    var cellPaddingY = asSize(style.cellPaddingY, defaults.style.cellPaddingY);
    var borderRadius = asSize(style.borderRadius, defaults.style.borderRadius);
    var buttonRadius = asSize(style.toolbarButtonRadius, defaults.style.toolbarButtonRadius);
    var buttonFontSize = asSize(style.toolbarButtonFontSize, defaults.style.toolbarButtonFontSize);
    var headerHeight = Number.isFinite(Number(layout.headerHeight)) ? Number(layout.headerHeight) : defaults.headerHeight;
    var showGrid = layout.showGrid !== false;
    var defaultColumnAlign = validateAlign(layout.defaultColumnAlign, defaults.defaultColumnAlign);
    var defaultHeaderAlign = validateAlign(layout.defaultHeaderAlign, defaults.defaultHeaderAlign);
    var gridBorderColor = showGrid ? style.border : "transparent";
    return [
      rootClass + " { width:100%; height:100%; min-height:80px; color:" + style.text + "; font-size:" + Number(style.fontSize) + "px; font-family:" + style.fontFamily + "; background:" + style.tableBackground + "; border-radius:" + borderRadius + "; box-shadow:" + style.tableShadow + "; }",
      rootClass + " { display:flex; flex-direction:column; box-sizing:border-box; overflow:hidden; }",
      rootClass + " .bst-toolbar { display:flex; flex:0 0 auto; gap:" + toolbarGap + "; align-items:center; padding:6px; background:" + style.toolbarBackground + "; border:1px solid " + gridBorderColor + "; border-bottom:0; box-sizing:border-box; }",
      rootClass + " .bst-toolbar button { border:1px solid " + style.toolbarButtonBorder + "; background:" + style.toolbarButtonBackground + "; color:" + style.toolbarButtonText + "; border-radius:" + buttonRadius + "; cursor:pointer; font:inherit; font-size:" + buttonFontSize + "; line-height:1; padding:6px 8px; transition:background-color 120ms ease, border-color 120ms ease, color 120ms ease; }",
      rootClass + " .bst-toolbar button:hover { background:" + style.toolbarButtonHoverBackground + "; border-color:" + style.accent + "; color:" + style.toolbarButtonText + "; }",
      rootClass + " .bst-toolbar button:focus-visible { outline:2px solid " + style.accent + "; outline-offset:1px; }",
      rootClass + " .bst-table-surface { flex:1 1 auto; min-height:0; width:100%; background:" + style.tableBackground + "; }",
      rootClass + " .bst-table-surface .tabulator { height:100%; background:" + style.tableBackground + "; border-radius:" + borderRadius + "; overflow:hidden; }",
      rootClass + " .bst-fallback-table { width:100%; border-collapse:collapse; table-layout:fixed; background:" + style.tableBackground + "; }",
      rootClass + " .bst-fallback-table th { background:" + style.headerBackground + "; color:" + style.headerText + "; border:1px solid " + gridBorderColor + "; height:" + headerHeight + "px; padding:" + cellPaddingY + " " + cellPaddingX + "; text-align:" + defaultHeaderAlign + "; font-weight:" + style.headerFontWeight + "; }",
      rootClass + " .bst-fallback-table td { background:" + style.rowBackground + "; color:" + style.text + "; border:1px solid " + gridBorderColor + "; height:" + Number(style.rowHeight) + "px; padding:" + cellPaddingY + " " + cellPaddingX + "; text-align:" + defaultColumnAlign + "; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }",
      rootClass + " .bst-fallback-table tbody tr:nth-child(even) td { background:" + style.alternateRowBackground + "; }",
      rootClass + " .bst-fallback-table tbody tr:hover td { background:" + style.rowHoverBackground + "; }",
      rootClass + " .bst-fallback-table tbody tr.is-selected td { outline:2px solid " + style.accent + "; outline-offset:-2px; }",
      rootClass + " .tabulator { border-color:" + gridBorderColor + "; color:" + style.text + "; font-size:" + Number(style.fontSize) + "px; font-family:" + style.fontFamily + "; }",
      rootClass + " .tabulator .tabulator-header { background:" + style.headerBackground + "; border-color:" + gridBorderColor + "; color:" + style.headerText + "; font-weight:" + style.headerFontWeight + "; min-height:" + headerHeight + "px; }",
      rootClass + " .tabulator .tabulator-header .tabulator-col { background:" + style.headerBackground + "; border-color:" + gridBorderColor + "; color:" + style.headerText + "; font-weight:" + style.headerFontWeight + "; height:" + headerHeight + "px; }",
      rootClass + " .tabulator .tabulator-header .tabulator-col .tabulator-col-content { padding:" + cellPaddingY + " " + cellPaddingX + "; }",
      ...
      rootClass + " .tabulator-row:hover { background:" + style.rowHoverBackground + "; }",
      rootClass + " .tabulator-row .tabulator-cell { border-color:" + gridBorderColor + "; padding:" + cellPaddingY + " " + cellPaddingX + "; min-height:" + Number(style.rowHeight) + "px; line-height:" + (Number(style.rowHeight) - Number(style.cellPaddingY) * 2) + "px; box-sizing:border-box; }",
      rootClass + " .tabulator-row.tabulator-selected { background:" + style.selectedRowBackground + "; }"
    ].join("\n");
  }

  function installScopedStyle(id, style, layout) {
    ...
    styleElement.textContent = buildStyleText(id, style, layout);
    return styleElement;
  }
  ...
  TableController.prototype.render = function (input) {
    ...
    this.styleElement = installScopedStyle(this.id, config.style, {
      ...
      defaultColumnAlign: config.defaultColumnAlign,
      defaultHeaderAlign: config.defaultHeaderAlign
    });
  ...
  TableController.prototype.renderTabulator = function () {
    ...
    var options = {
      ...
      columnDefaults: {
        ...
        headerSort: true,
        hozAlign: this.config.defaultColumnAlign,
        headerHozAlign: this.config.defaultHeaderAlign,
        width: this.config.defaultColumnWidth || undefined
      }
    };
  ...
  TableController.prototype.renderFallback = function () {
    ...
    var fallbackDefaultWidth = self.config.defaultColumnWidth;
    this.columns.forEach(function (column) {
      var th = root.document.createElement("th");
      th.textContent = column.title || column.field;
      var widthValue = column.width != null ? column.width : fallbackDefaultWidth;
      if (widthValue) th.style.width = typeof widthValue === "number" ? widthValue + "px" : widthValue;
      if (column.headerHozAlign) th.style.textAlign = column.headerHozAlign;
    ...
    this.rows.forEach(function (row, rowIndex) {
      ...
      self.columns.forEach(function (column) {
        ...
        if (column.hozAlign) td.style.textAlign = column.hozAlign;
  ...
  function readBubbleProperties(properties) {
    ...
    return {
      ...
      defaultColumnAlign: properties.default_column_align,
      defaultHeaderAlign: properties.default_header_align,
      defaultColumnWidth: properties.default_column_width,
      headerHeight: properties.header_height,
      showGrid: properties.show_grid,
      style: {
        ...
        headerFontWeight: properties.header_font_weight,
        rowBackground: properties.row_background_color,
        alternateRowBackground: properties.alternate_row_background_color,
        rowHoverBackground: properties.row_hover_background_color,
        selectedRowBackground: properties.selected_row_background_color,
        ...
        cellPaddingX: properties.cell_padding_x,
        cellPaddingY: properties.cell_padding_y,
        toolbarBackground: properties.toolbar_background_color,
        toolbarButtonBackground: properties.toolbar_button_background_color,
        toolbarButtonText: properties.toolbar_button_text_color,
        toolbarButtonHoverBackground: properties.toolbar_button_hover_background_color,
        toolbarButtonBorder: properties.toolbar_button_border_color,
        toolbarButtonRadius: properties.toolbar_button_radius,
        toolbarButtonFontSize: properties.toolbar_button_font_size,
        toolbarGap: properties.toolbar_gap,
        tableBackground: properties.table_background_color,
        tableShadow: properties.table_shadow
      },
      styleOverrides: parseJsonInput(properties.style_overrides_json, {}, "style_overrides_json")
    };
  }
